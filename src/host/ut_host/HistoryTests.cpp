...
class HistoryTests
{
    ...
    TEST_METHOD(AllocateAndFreeOneApp)
    {
        ...
        VERIFY_IS_TRUE(IsFlagSet(history->Flags, CLE_ALLOCATED));
        ...
        VERIFY_IS_TRUE(IsFlagClear(history->Flags, CLE_ALLOCATED), L"Shouldn't actually be gone, just deallocated.");
        VERIFY_ARE_EQUAL(1ul, CommandHistory::s_historyLists.size());
    }
