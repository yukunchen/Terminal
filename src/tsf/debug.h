...
#undef __DEBUG_UNICODE  // Debug code should be ASCII

#ifdef __DEBUG_UNICODE
#define __DEBUG_TCHAR    wchar_t
#define __DEBUG_TEXT(x)  L##x
#else
#define __DEBUG_TCHAR    char
#define __DEBUG_TEXT(x)  x
#endif
...
// General flag macros
//
#define DbgSetFlag(obj, f)             do {obj |= (f);} while (0)
#define DbgToggleFlag(obj, f)          do {obj ^= (f);} while (0)
#define DbgClearFlag(obj, f)           do {obj &= ~(f);} while (0)
#define DbgIsFlagSet(obj, f)           (BOOL)(((obj) & (f)) == (f))
#define DbgIsFlagClear(obj, f)         (BOOL)(((obj) & (f)) != (f))
...
#define DEBUGTEXT(sz, msg)      /* ;Internal */ \
    static const __DEBUG_TCHAR sz[] = msg
...
#define ASSERT(f)                                 \
    {                                             \
        DEBUGTEXT(szFile, __DEBUG_TEXT(__FILE__));              \
        if (!(f) && CcshellAssertFailed(szFile, __LINE__, __DEBUG_TEXT(#f), FALSE, TRUE)) \
            DEBUG_BREAK;       \
    }
...
#define ASSERTPRIVATE(f)                                 \
    {                                             \
        DEBUGTEXT(szFile, __DEBUG_TEXT(__FILE__));              \
        if (!(f) && CcshellAssertFailed(szFile, __LINE__, __DEBUG_TEXT(#f), FALSE, FALSE)) \
            DEBUG_BREAK;       \
    }
...
#define EVAL(exp)   \
    ((exp) || (CcshellAssertFailed(__DEBUG_TEXT(__FILE__), __LINE__, __DEBUG_TEXT(#exp), TRUE, FALSE), 0))
...
#define RIP(f)                                                                                              \
    {                                                                                                       \
        DEBUGTEXT(szFile, __DEBUG_TEXT(__FILE__));                                                                        \
        if (!(f) && CcshellRip(szFile, __LINE__, __DEBUG_TEXT(#f), FALSE))                                                \
        {                                                                                                   \
            DEBUG_BREAK;                                                                                    \
        }                                                                                                   \
    }                                                                                                       \
...
#define RIPMSG(f, msg)                                                                                      \
    {                                                                                                       \
        DEBUGTEXT(szFile, __DEBUG_TEXT(__FILE__));                                                                  \
        if (!(f) && CcshellRip(szFile, __LINE__, __DEBUG_TEXT(#msg), FALSE))                                        \
        {                                                                                                   \
            DEBUG_BREAK;                                                                                    \
        }                                                                                                   \
    }                                                                                                       \
