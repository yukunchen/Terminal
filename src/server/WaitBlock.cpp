...
// - pvWaitParameter - Parameter to send to callback routine when calling
...
ConsoleWaitBlock::ConsoleWaitBlock(_In_ ConsoleWaitQueue* const pProcessQueue,
    ...
    _pvWaitParameter(pvWaitParameter)
{
    ...
    memmove(&_WaitReplyMessage, pWaitReplyMessage, sizeof(_WaitReplyMessage));
...
// - pvWaitParameter - Parameter to pass to callback routine when it is triggered later.
// Return Value:
...
{
    if ((_pfnWaitRoutine)(&_WaitReplyMessage, _pvWaitParameter, TerminationReason))
    {
