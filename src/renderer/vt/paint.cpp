...
HRESULT VtEngine::_PrepareMemoryBitmap(_In_ HWND const hwnd)
{
    RECT rcClient;
...
// Return Value:
// - S_OK or suitable GDI HRESULT error.
HRESULT VtEngine::_PaintBackgroundColor(_In_ const RECT* const prc)
{
...
HRESULT VtEngine::_FlushBufferLines()
{
    HRESULT hr = S_OK;
...
HRESULT VtEngine::_PaintSelectionCalculateRegion(_In_reads_(cRectangles) SMALL_RECT* const rgsrSelection,
...
// Return Value:
// - <none>
void VtEngine::_PaintDebugRect(_In_ const RECT* const prc) const
{
    if (_fDebug)
    {
        if (!IsRectEmpty(prc))
...
// Return Value:
// - <none>
void VtEngine::_DoDebugBlt(_In_ const RECT* const prc) const
{
    if (_fDebug)
    {
        if (!IsRectEmpty(prc))
