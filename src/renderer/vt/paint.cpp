                                               ...
                                               _In_ const WORD cColorTable)
{
    if (colorForeground != _LastFG)
    {
        if (colorForeground == _colorProvider.GetDefaultForeground())
        {
            // Reset the colors (to defaults) then set the BG back to what it was before.
            RETURN_IF_FAILED(_SetGraphicsRenditionResetColors());
            const WORD wNearestBg = ::FindNearestTableIndex(_LastBG, ColorTable, cColorTable);
    ...
    if (colorBackground != _LastBG)
    {
        if (colorBackground == _colorProvider.GetDefaultBackground())
        {
            // Reset the colors (to defaults) then set the FG back to what it was before.
            RETURN_IF_FAILED(_SetGraphicsRenditionResetColors());
            const WORD wNearestFg = ::FindNearestTableIndex(_LastFG, ColorTable, cColorTable);
            RETURN_IF_FAILED(_SetGraphicsRendition16Color(wNearestFg, true));
        }
        else
        {
