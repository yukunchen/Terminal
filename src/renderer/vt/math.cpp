...
HRESULT VtEngine::_ScaleByFont(_In_ const SMALL_RECT* const psr, _Out_ RECT* const prc) const
{
...
// Return Value:
// - S_OK or safe math failure value.
HRESULT VtEngine::_ScaleByFont(_In_ const COORD* const pcoord, _Out_ POINT* const pPoint) const
{
    COORD const coordFontSize = _GetFontSize();
    RETURN_HR_IF(HRESULT_FROM_WIN32(ERROR_INVALID_STATE), coordFontSize.X == 0 || coordFontSize.Y == 0);
...
// Return Value:
// - S_OK or safe math failure value.
HRESULT VtEngine::_ScaleByFont(_In_ const RECT* const prc, _Out_ SMALL_RECT* const psr) const
{
    COORD const coordFontSize = _GetFontSize();
    RETURN_HR_IF(HRESULT_FROM_WIN32(ERROR_INVALID_STATE), coordFontSize.X == 0 || coordFontSize.Y == 0);
...
int VtEngine::s_ScaleByDpi(_In_ const int iPx, _In_ const int iDpi)
{
...
int VtEngine::s_ShrinkByDpi(_In_ const int iPx, _In_ const int iDpi)
{
...
POINT VtEngine::_GetInvalidRectPoint() const
{
    POINT pt;
...
SIZE VtEngine::_GetInvalidRectSize() const
{
...
SIZE VtEngine::_GetRectSize(_In_ const RECT* const pRect) const
{
    SIZE sz;
...
// Return Value:
// - <none>
void VtEngine::_OrRect(_In_ RECT* const pRectExisting, _In_ const RECT* const pRectToOr) const
{
