...
#pragma region FILETIME helpers
    ...
    namespace FileTimeDuration
    {
        ...
        long long const OneMinute      = 10000000LL * 60;           // 600000000    or 600000000LL
        long long const OneHour        = 10000000LL * 60 * 60;      // 36000000000  or 36000000000LL
        long long const OneDay         = 10000000LL * 60 * 60 * 24; // 864000000000 or 864000000000LL
    };
...
#pragma endregion
    ...
    template <typename string_type, size_t stackBufferLength = 256>
    HRESULT QueryFullProcessImageName(HANDLE processHandle, _In_ DWORD flags, string_type& result) WI_NOEXCEPT
    {
        return wil::AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(result,
            [&](_Out_writes_(valueLength) PWSTR value, size_t valueLength, _Out_ size_t* valueLengthNeededWithNul) -> HRESULT
        {
    ...
    HRESULT GetModuleFileNameExW(_In_opt_ HANDLE process, _In_opt_ HMODULE module, string_type& path)
    {
        ...
        // Any path should fit into the maximum maxExtendedPathLength. If we reached here, something went
        // terribly wrong.
        FAIL_FAST();
    }
