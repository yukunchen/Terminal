...
#define WilComFinal
#else
//! Use as an alternative to the `final` keyword on COM objects that will be held by `wil::com_ptr`.
//! The smart pointer classes within wil utilize a technique of creating a thin wrapper that derives from the object
//! being held to prevent callers from accidentally directly using AddRef or Release on the underlying COM object (likely creating
//! a bug along the way).  The final keyword gets in the way of doing that, so by using `WrlComFinal`, the final
//! behavior can be stripped out in a checked build to validate that AddRef or Release are not being incorrectly called, yet be present
//! in a retail build.
#define WilComFinal final
#endif
...
namespace wil
{
    ...
    namespace details
    {
        // Prevent com_ptr<> callers from making direct AddRef/Release calls.
        // note: do not mark methods virtual to avoid v-table bloat
        template <typename T>
        class no_IUnknown : public T
        {
        private:
            ~no_IUnknown();
            // When you see this kind of error:
            // error C2248: 'wil::details::remove_IUnknown<T>::QueryInterface' : cannot access private member declared in class 'wil::details::remove_IUnknown<T>'
            // You are trying to call THROUGH a com_ptr<> to an IUnknown method, rather than using the methods of the class to manage the pointer (query, attach, detach, etc)
            HRESULT __stdcall QueryInterface(REFIID riid, _COM_Outptr_ void **ppvObject);
            ULONG __stdcall AddRef();
            ULONG __stdcall Release();
        };

        template <typename T, bool isConstT>
        struct remove_IUnknown_helper
        {
            typedef const no_IUnknown<T> type;
        };

        template <typename T>
        struct remove_IUnknown_helper<T, false>
        {
            typedef no_IUnknown<T> type;
        };

        template <typename T>
        struct remove_IUnknown
        {
            typedef typename remove_IUnknown_helper<T, wistd::is_const<T>::value>::type type;
        };

        // Note that at one point this was done with __is_final type traits, but __is_final prevents using a com_ptr<ClassName> against a forward
        // declaration of ClassName.
#ifdef WI_DEBUG
        template <typename T>
        using remove_IUnknown_t = typename remove_IUnknown<T>::type;
#else
        template <typename T>
        using remove_IUnknown_t = T;
#endif
        ...
        struct is_com_convertible :
            wistd::_Cat_base<__is_convertible_to(TFrom, TTo) && (__is_abstract(TFrom) || wistd::is_same<TFrom, TTo>::value)>
        {
        };
        ...
        public:
            ...
            {
                return query_dispatch(ptr, details::is_com_convertible<T*, TResult*>::type(), result);
            }
    ...
    {
    private:
        typedef typename details::remove_IUnknown_t<T>* pointer_no_unknown;
        typedef typename wistd::add_reference<details::remove_IUnknown_t<T>>::type element_type_reference_no_unknown;
        typedef typename details::query_policy_t<T> query_policy;
    public:
        ...
        template <class U, typename err, class = typename wistd::enable_if_t<__is_convertible_to(U*, pointer)>>
        ...
        _Check_return_ pointer detach() WI_NOEXCEPT
        {
            auto temp = m_ptr;
            m_ptr = nullptr;
            return temp;
        ...
        //! Returns the const address of the internal pointer (does not release the pointer)
        pointer const * addressof() const WI_NOEXCEPT
        {
            return &m_ptr;
        }
        ...
        //! Allows direct calls against the pointer (suppresses IUnknown methods, AV on internal nullptr)
        pointer_no_unknown operator->() const WI_NOEXCEPT
        {
            return static_cast<pointer_no_unknown>(m_ptr);
        }

        //! Dereferences the pointer (suppresses IUnkown methods, AV on internal nullptr)
        element_type_reference_no_unknown operator*() const WI_NOEXCEPT
        {
            return static_cast<element_type_reference_no_unknown>(*m_ptr);
        }
        //! @}
        ...
        template <class U, class = typename wistd::enable_if_t<__is_convertible_to(U*, pointer)>>
    ...
    //! An overloaded method that retrieves the raw com pointer from a raw pointer, wil::com_ptr_t<T>, or a WRL ComPtr<T>.
    //! This method is primarily useful by library or helper code.  It allows code to be written to accept a generic R-Value reference
...
#pragma region Stream helpers
    ...
    */
    inline HRESULT stream_write_nothrow(_In_ ISequentialStream* stream, _In_reads_bytes_(size) const void* data, unsigned long size)
    {
        ...
        RETURN_HR_IF_FALSE(HRESULT_FROM_WIN32(ERROR_INVALID_DATA), wrote == size);
    ...
    */
    inline HRESULT stream_seek_nothrow(_In_ IStream* stream, long long offset, unsigned long from, _Out_opt_ unsigned long long* value = nullptr)
    {
        ...
        AssignToOptParam(value, landed.QuadPart);
    ...
    */
    inline HRESULT stream_copy_bytes_nothrow(_In_ IStream* source, _In_ IStream* target, unsigned long long amount, _Out_opt_ unsigned long long* pCopied = nullptr)
    {
        ...
        RETURN_IF_FAILED(source->CopyTo(target, toCopy, nullptr, pCopied ? &copied : nullptr));
        AssignToOptParam(pCopied, copied.QuadPart);
    ...
    */
    inline HRESULT stream_copy_exact_nothrow(_In_ IStream* source, _In_ IStream* target, unsigned long long amount)
    {
        ...
        RETURN_HR_IF_FALSE(HRESULT_FROM_WIN32(ERROR_INVALID_DATA), copied == amount);
    ...
    */
    ...
    {
        THROW_HR_IF_FALSE(HRESULT_FROM_WIN32(ERROR_INVALID_DATA), stream_read_partial(stream, data, size) == size);
    }
    ...
    */
    ...
    {
        THROW_HR_IF_FALSE(HRESULT_FROM_WIN32(ERROR_INVALID_DATA), stream_copy_bytes(source, target, amount) == amount);
    }
