...
NTSTATUS RegistrySerialization::s_LoadRegDword(_In_ HANDLE const hKey, _In_ const _RegPropertyMap* const pPropMap, _In_ Settings* const pSettings)
{
...
// Return Value:
// - STATUS_SUCCESSFUL or appropriate NTSTATUS reply for registry operations.
NTSTATUS RegistrySerialization::s_LoadRegString(_In_ HANDLE const hKey, _In_ const _RegPropertyMap* const pPropMap, _In_ Settings* const pSettings)
{
    // find offset into destination structure for this numerical value
    PBYTE const pbField = (PBYTE)pSettings + pPropMap->dwFieldOffset;
...
// - STATUS_SUCCESSFUL or appropriate NTSTATUS reply for registry operations.
_Check_return_
NTSTATUS RegistrySerialization::s_OpenConsoleKey(_Out_ HANDLE* phCurrentUserKey, _Out_ HANDLE* phConsoleKey)
{
    NTSTATUS Status = STATUS_SUCCESS;

    // Open the current user registry key.
    LONG Result = RegOpenKeyW(HKEY_CURRENT_USER,
                              nullptr,
                              (HKEY*)phCurrentUserKey);
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        if (!NT_SUCCESS(Status))
        {
            RegCloseKey((HKEY)*phCurrentUserKey);
        }
    }
...
// - STATUS_SUCCESSFUL or appropriate NTSTATUS reply for registry operations.
_Check_return_
NTSTATUS RegistrySerialization::s_OpenKey(_In_opt_ HANDLE const hKey, _In_ PCWSTR const pwszSubKey, _Out_ HANDLE* const phResult)
{
    NTSTATUS Status = STATUS_SUCCESS;

    LONG Result = RegOpenKeyW((HKEY)hKey,
                              pwszSubKey,
                              (HKEY*)phResult);
...
// Routine Description:
// - Deletes the subkey of a given key
// Arguments:
// - hKey - Handle to a registry key
// - pwszSubKey - String name of sub key
// Return Value:
// - STATUS_SUCCESSFUL or appropriate NTSTATUS reply for registry operations.
_Check_return_
NTSTATUS RegistrySerialization::s_DeleteKey(_In_ HANDLE const hKey, _In_ PCWSTR const pwszSubKey)
{
    NTSTATUS Status = STATUS_SUCCESS;

    LONG Result = RegDeleteKeyW((HKEY)hKey,
                                pwszSubKey);
...
// - STATUS_SUCCESSFUL or appropriate NTSTATUS reply for registry operations.
_Check_return_
NTSTATUS RegistrySerialization::s_CreateKey(_In_ HANDLE const hKey, _In_ PCWSTR const pwszSubKey, _Out_ HANDLE* const phResult)
{
    NTSTATUS Status = STATUS_SUCCESS;

    LONG Result = RegCreateKeyW((HKEY)hKey,
                                pwszSubKey,
                                (HKEY*)phResult);
...
// - STATUS_SUCCESSFUL or appropriate NTSTATUS reply for registry operations.
_Check_return_
NTSTATUS RegistrySerialization::s_SetValue(_In_ HANDLE const hKey,
                                           ...
                                           _In_ DWORD const cbDataLength)
{
    ...
    LONG Result = RegSetKeyValueW((HKEY)hKey,
                                  nullptr,
                                  pValueName,
                                  dwType,
                                  pbData,
...
// - STATUS_SUCCESSFUL or appropriate NTSTATUS reply for registry operations.
_Check_return_
NTSTATUS RegistrySerialization::s_QueryValue(_In_ HANDLE const hKey,
                                             ...
                                             _Out_opt_ _Out_range_(0, cbValueLength) DWORD* const pcbDataLength)
{
    ...
    LONG Result = RegQueryValueExW((HKEY)hKey,
                                   pwszValueName,
                                   nullptr,
                                   nullptr,
                                   pbData,
...
// - STATUS_SUCCESSFUL or appropriate NTSTATUS reply for registry operations.
_Check_return_
NTSTATUS RegistrySerialization::s_EnumValue(_In_ HANDLE const hKey,
                                            ...
                                            _Out_writes_bytes_(cbDataLength) BYTE* const pbData)
{
    ...
    LONG Result = RegEnumValueW((HKEY)hKey,
                                dwIndex,
                                pwszValueName,
                                &cchValueName,
                                nullptr,
...
// - STATUS_SUCCESSFUL or appropriate NTSTATUS reply for registry operations.
_Check_return_
NTSTATUS RegistrySerialization::s_UpdateValue(_In_ HANDLE const hConsoleKey,
                                              ...
                                              _In_ DWORD const cbDataLength)
{
    ...
    if (Data != nullptr)
    {
        ...
        if (fDeleteKey)
        {
            Status = s_DeleteKey(hKey, pwszValueName);
        }
        else
        {
...
NTSTATUS RegistrySerialization::s_OpenCurrentUserConsoleTitleKey(_In_ PCWSTR const title,
                                                                 _Out_ HANDLE* phCurrentUserKey,
                                                                 _Out_ HANDLE* phConsoleKey,
                                                                 _Out_ HANDLE* phTitleKey)
{
    NTSTATUS Status = STATUS_SUCCESS;
...
extern "C"
{
    ...
    NTSTATUS RegistrySerializationOpenKey(_In_opt_ HANDLE const hKey, _In_ PCWSTR const pwszSubKey, _Out_ HANDLE* const phResult)
    {
        return RegistrySerialization::s_OpenKey(hKey, pwszSubKey, phResult);
    }


    NTSTATUS RegistrySerializationQueryValue(_In_ HANDLE const hKey,
                                             _In_ PCWSTR const pwszValueName,
                                             _In_ DWORD const cbValueLength,
                                             _Out_writes_bytes_(cbValueLength) BYTE* const pbData,
                                             _Out_opt_ _Out_range_(0, cbValueLength) DWORD* const pcbDataLength)
    ...
    NTSTATUS RegistrySerializationEnumValue(_In_ HANDLE const hKey,
                                            _In_ DWORD const dwIndex,
                                            _In_ DWORD const cbValueLength,
                                            _Out_writes_bytes_(cbValueLength) PWSTR const pwszValueName,
                                            _In_ DWORD const cbDataLength,
    ...
    NTSTATUS RegistrySerializationOpenConsoleKey(_Out_ HANDLE* phCurrentUserKey, _Out_ HANDLE* phConsoleKey)
    {
        return RegistrySerialization::s_OpenConsoleKey(phCurrentUserKey, phConsoleKey);
    }


    NTSTATUS RegistrySerializationCreateKey(_In_ HANDLE const hKey, _In_ PCWSTR const pwszSubKey, _Out_ HANDLE* const phResult)
    {
        return RegistrySerialization::s_CreateKey(hKey, pwszSubKey, phResult);
    }


    NTSTATUS RegistrySerializationDeleteKey(_In_ HANDLE const hKey, _In_ PCWSTR const pwszSubKey)
    {
        return RegistrySerialization::s_DeleteKey(hKey, pwszSubKey);
    }


    NTSTATUS RegistrySerializationSetValue(_In_ HANDLE const hKey,
                                           _In_ PCWSTR const pwszValueName,
                                           _In_ DWORD const dwType,
                                           _In_reads_bytes_(cbDataLength) BYTE* const pbData,
                                           _In_ DWORD const cbDataLength)
    ...
    NTSTATUS RegistrySerializationUpdateValue(_In_ HANDLE const hConsoleKey,
                                              _In_ HANDLE const hKey,
                                              _In_ PCWSTR const pwszValueName,
                                              _In_ DWORD const dwType,
