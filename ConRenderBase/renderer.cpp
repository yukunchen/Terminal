...
#include "..\ConHostV2\cursor.h"
#include "..\ConHostV2\textBuffer.hpp"
#include "..\ConHostV2\conimeinfo.h"
...
void Renderer::_PaintBufferOutputColorHelper(_In_ const ROW* const pRow, _In_reads_(cchLine) PCWCHAR const pwsLine, _In_reads_(cchLine) PBYTE pbKAttrsLine, _In_ size_t cchLine, _In_ size_t iFirstAttr, _In_ COORD const coordTarget)
{
    ...
    do
    {
        ...
        ATTR_PAIR* pAttr = nullptr;
        UINT cAttrApplies = 0;
        pRow->AttrRow.FindAttrIndex((UINT)(iFirstAttr + cchWritten), &pAttr, &cAttrApplies);

        // Set the brushes in GDI to this color
        _UpdateDrawingBrushes(pAttr->Attr, false);
        ...
        if (_pData->IsGridLineDrawingAllowed())
        {
            ...
            _PaintBufferOutputGridLineHelper(pAttr->Attr, cchSegment, coordOffset);
        }
...
// Return Value:
// - <none>
void Renderer::_PaintBufferOutputGridLineHelper(_In_ WORD const wAttr, _In_ size_t const cchLine, _In_ COORD const coordTarget)
{
    // Reverse if necessary.
    COLORREF rgb;
    if ((wAttr & COMMON_LVB_REVERSE_VIDEO) == 0)
    {
        rgb = _ConvertAttrToRGB(LOBYTE(wAttr));
    }
    else
    {
        rgb = _ConvertAttrToRGB(LOBYTE(wAttr >> 4));
    }
    ...
    if ((wAttr & COMMON_LVB_GRID_HORIZONTAL) != 0)
    {
        lines |= IRenderEngine::GridLines::Top;
    }

    if ((wAttr & COMMON_LVB_UNDERSCORE) != 0)
    {
        lines |= IRenderEngine::GridLines::Bottom;
    }

    if ((wAttr & COMMON_LVB_GRID_LVERTICAL) != 0)
    {
        lines |= IRenderEngine::GridLines::Left;
    }

    if ((wAttr & COMMON_LVB_GRID_RVERTICAL) != 0)
    {
...
// Routine Description:
// - Paint helper to draw any particular portion of the IME within the buffer.
// - This supports all 3 areas (composition, system, and mode).
// Arguments:
// - pAreaInfo - Special IME area screen buffer metadata
// - pTextInfo - Text backing buffer for each special IME area.
// Return Value:
// - <none>
...
// Return Value:
// - <none>
void Renderer::_UpdateDrawingBrushes(_In_ WORD const wTextAttributes, _In_ bool const fIncludeBackground)
{
    // Convert attributes into actual RGB colors
    COLORREF rgbForeground = _ConvertAttrToRGB(LOBYTE(wTextAttributes));
    COLORREF rgbBackground = _ConvertAttrToRGB(LOBYTE(wTextAttributes >> 4));

    // If we have the reverse video flag set, swap foreground & background
    if ((wTextAttributes & COMMON_LVB_REVERSE_VIDEO) != 0)
    {
        COLORREF const rgbSwap = rgbForeground;
        rgbForeground = rgbBackground;
        rgbBackground = rgbSwap;
    }
