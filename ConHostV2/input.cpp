...
#include "..\ConTermAdapt\terminalInput.hpp"
...
NTSTATUS CreateInputBuffer(_In_opt_ ULONG cEvents, _Out_ PINPUT_INFORMATION pInputInfo)
{
    ...
    ULONG uTemp;
    if (!SUCCEEDED(DWordAdd(cEvents, 1, &uTemp)) || !SUCCEEDED(DWordMult(sizeof(INPUT_RECORD), uTemp, &uTemp)))
    {
        cEvents = DEFAULT_NUMBER_OF_EVENTS;
    }
                           ...
                           _In_ const BOOLEAN fWaitBlockExists)
{
    if (!fWaitBlockExists)
    {
        ...
        CopyMemory(WaitParameterBuffer, pvWaitParameter, cbWaitParameter);
...
NTSTATUS FlushAllButKeys()
{
    ...
    if (InputInformation->In != InputInformation->Out)
    {
        ...
        if (!SUCCEEDED(DWordMult(sizeof(INPUT_RECORD), InputInformation->InputBufferSize + 1, &BufferSize)))
        {
...
NTSTATUS SetInputBufferSize(_Inout_ PINPUT_INFORMATION InputInformation, _In_ ULONG Size)
{
    ...
    if (!SUCCEEDED(SizeTMult(sizeof(INPUT_RECORD), Size + 1, &BufferSize)))
    {
        return STATUS_INTEGER_OVERFLOW;
    }
           ...
           _In_ BOOLEAN Unicode)
{
    *ResetWaitEvent = FALSE;
    ...
    if (StreamRead && ((PINPUT_RECORD) (InputInformation->Out))->EventType == KEY_EVENT)
    {
        ...
        MoveMemory((PBYTE) Buffer, (PBYTE) InputInformation->Out, sizeof(INPUT_RECORD));
    ...
    if (InputInformation->In > InputInformation->Out)
    {
        ...
        {
            MoveMemory((PBYTE) Buffer, (PBYTE) InputInformation->Out, TransferLength);
        }
    ...
    else
    {
        ...
        *EventsRead = TransferLength / sizeof(INPUT_RECORD);
        ...
        {
            MoveMemory((PBYTE) Buffer + OldTransferLength, (PBYTE) InputInformation->First, TransferLength);
        }
    ...
    *EventsWritten = 0;
    while (*EventsWritten < Length)
    {
        ...
        if (InputInformation->Out > InputInformation->In)
        {
            ...
            MoveMemory((PBYTE) InputInformation->In, (PBYTE) Buffer, TransferLength);
    ...
    for (ULONG NumEvents = nLength; NumEvents != 0; NumEvents--)
    {
        if (InputEvent->EventType == KEY_EVENT && InputEvent->Event.KeyEvent.bKeyDown)
        {
            ...
            if ((g_ciConsoleInformation.Flags & CONSOLE_SUSPENDED) && !IsSystemKey(InputEvent->Event.KeyEvent.wVirtualKeyCode))
            {
                ...
                MoveMemory(InputEvent, InputEvent + 1, (NumEvents - 1) * sizeof(INPUT_RECORD));
                nLength--;
                continue;
            }
...
NTSTATUS PrependInputBuffer(_In_ PINPUT_INFORMATION pInputInfo, _In_ PINPUT_RECORD pInputRecord, _Inout_ DWORD * const pcLength)
{
    ...
    if (NumExistingEvents)
    {
        ...
        if (!SUCCEEDED(ULongMult(NumExistingEvents, sizeof(INPUT_RECORD), &NumBytes)))
        {
            return STATUS_INTEGER_OVERFLOW;
        }
...
void InitSideBySide(_Out_writes_(ScratchBufferSize) PWSTR ScratchBuffer, __range(MAX_PATH, MAX_PATH) DWORD ScratchBufferSize)
{
    ...
    // This is similar to code in base\ntsetup\syssetup\copy.c

    // These will both be true, unless conhost is at some future
    // date launched with CreateProcess or SideBySide support moved
    // into the kernel and SideBySide setup moved to textmode, at which
    // time this code block will not be needed.
    // TODO: I'm honestly not sure this is even necessary at all. It doesn't seem to do anything of value except check if 
    //       the console is *IN* the SxS cache. It doesn't put it there, it just tries to activate it. So why???
 /*   if (Peb->SystemDefaultActivationContextData != nullptr || Peb->ActivationContextData != nullptr)
    {
        goto Exit;
    }*/
...
DWORD ConsoleInputThread(LPVOID /*lpParameter*/)
{
    ...
    if (!IsIMEInputSupported())
    {
        ImmAssociateContext(g_ciConsoleInformation.hWnd, nullptr);
        g_ciConsoleInformation.pInputBuffer->ImeMode.Disable = TRUE;
    }
...
BOOL HandleMouseEvent(_In_ const SCREEN_INFORMATION * const pScreenInfo, _In_ const UINT Message, _In_ const WPARAM wParam, _In_ const LPARAM lParam)
{
    ...
    if (pSelection->IsInSelectingState() || pSelection->IsInQuickEditMode())
    {
        ...
        else if (Message == WM_LBUTTONDBLCLK)
        {
            ...
            {
                PROW const Row = pScreenInfo->TextInfo->GetRowByOffset(MousePosition.Y);
                ASSERT(Row != nullptr);
