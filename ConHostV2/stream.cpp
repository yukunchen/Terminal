...
bool IsDbcsExemptionForHighAnsi(_In_ UINT const wCodePage, _In_ WORD const wNumpadChar)
{
    ASSERT(HIBYTE(wNumpadChar) == 0);

    if (wCodePage == CP_JAPANESE && IS_JPN_1BYTE_KATAKANA(wNumpadChar))
    {
        // If hkl is JAPANESE and NumpadChar is in KANA range, NumpadChar should be handled by the input locale.
        return false;
    }
    else if (wNumpadChar >= 0x80 && wNumpadChar <= 0xff)
    {
        // Otherwise if NumpadChar is in High ANSI range, use 1252 (US) for conversion.
        return true;
    }

    // None of the above.
    // This case includes the compound Leading Byte and Trailing Byte, which is larger than 0xff.
    return false;
}
...
{
    if (ARGUMENT_PRESENT(pfCommandLineEditingKeys))
    {
        *pfCommandLineEditingKeys = FALSE;
    }

    if (ARGUMENT_PRESENT(pfCommandLinePopupKeys))
    {
        *pfCommandLinePopupKeys = FALSE;
    }

    if (ARGUMENT_PRESENT(pfEnableScrollMode))
    {
        *pfEnableScrollMode = FALSE;
    }

    if (ARGUMENT_PRESENT(pdwKeyState))
    {
        *pdwKeyState = 0;
    }
    ...
    for (;;)
    {
        ...
        if (Event.EventType == KEY_EVENT)
        {
            ...
            else if (ARGUMENT_PRESENT(pfCommandLinePopupKeys))
            {
                ...
                if (!Event.Event.KeyEvent.bKeyDown && Event.Event.KeyEvent.wVirtualKeyCode == VK_MENU)
                {
                    if (Event.Event.KeyEvent.dwControlKeyState & ALTNUMPAD_BIT)
                    {
                        if (HIBYTE(Event.Event.KeyEvent.uChar.UnicodeChar))
                        {
                            ...
                            *pwchOut = CharToWchar(g_ciConsoleInformation.CP, chT, 2);
                        }
                        else
                        {
                            ...
                            UINT uCodePage = g_ciConsoleInformation.CP;

                            // East Asian hack for High ANSI OEM characters.
                            if (IsDbcsExemptionForHighAnsi(uCodePage, chT))
                            {
                                // East Asian hack:
                                // treat characters in High ANSI area as if they are the ones of Codepage 1252 (US).
                                uCodePage = CP_USA;
                            }
                            *pwchOut = CharToWchar(uCodePage, &chT, 1);
                        }
                    }
                    else
                    {
            ...
            if (Event.Event.KeyEvent.bKeyDown)
            {
                ...
                if (ARGUMENT_PRESENT(pfCommandLineEditingKeys) && fCommandLineEditKey)
                {
                    *pfCommandLineEditingKeys = TRUE;
                    *pwchOut = (WCHAR)Event.Event.KeyEvent.wVirtualKeyCode;
                    return STATUS_SUCCESS;
                }
                else if (ARGUMENT_PRESENT(pfCommandLinePopupKeys) && fCommandLineEditKey)
                {
    ...
    } __finally
    {
        ...
        if (Status != CONSOLE_STATUS_WAIT && !fSkipFinally)
        {
            if (!a->Unicode)
            {
                ...
                CopyMemory(lpBuffer, TransBuffer, a->NumBytes);
                if (fAddDbcsLead)
                {
                    a->NumBytes++;
                }
                   ...
                   _In_ BOOLEAN const fUnicode)
{
    ...
    *pdwNumBytes = 0;
    ...
    if (pHandleData->pClientInput->InputHandleFlags & HANDLE_INPUT_PENDING)
    {
        ...
        CopyMemory(pwchBuffer, pHandleData->pClientInput->CurrentBufPtr, NumToWrite);
...
#pragma prefast(suppress:__WARNING_POTENTIAL_BUFFER_OVERFLOW_HIGH_PRIORITY, "This access is fine but prefast can't follow it, evidently")
            CopyMemory(pwchBuffer, TransBuffer, NumToWrite);
...
#pragma prefast(suppress:26053 26015, "PREfast claims read overflow. *pdwNumBytes is the exact size of TransBuffer as allocated above.")
                CopyMemory(pwchBuffer, TransBuffer, *pdwNumBytes);
