...
#include "..\ConRenderBase\renderer.hpp"
#include "..\ConRenderGdi\gdirenderer.hpp"
    ...
    if (NT_SUCCESS(status))
    {
        ...
        if (NT_SUCCESS(status))
        {
            ...
            if (NT_SUCCESS(status))
            {
                ...
                // Disable the IMM32 default input context, if IME input not supported.
                                      ...
                                      _In_ const Window* const pWindow)
{
    ...
    HANDLE hCurrentUserKey, hConsoleKey, hTitleKey;
