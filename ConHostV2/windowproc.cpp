...
LRESULT CALLBACK Window::ConsoleWindowProc(_In_ HWND hWnd, _In_ UINT Message, _In_ WPARAM wParam, _In_ LPARAM lParam)
{
    ...
    case WM_GETDPISCALEDSIZE:
    {
        ...
        SMALL_RECT srViewport = ScreenInfo->BufferViewport;
    ...
    {
        g_ciConsoleInformation.hklActive = (HKL)lParam;

        if ((g_ciConsoleInformation.Flags & CONSOLE_HAS_FOCUS) && !CONSOLE_IS_TSF_ACTIVE() && !g_ciConsoleInformation.pInputBuffer->ImeMode.Disable)
        {
            if (ActivateTextServices(g_ciConsoleInformation.hWnd, GetImeSuggestionWindowPos))
            {
                g_ciConsoleInformation.Flags |= CONSOLE_TSF_ACTIVATED;
            }
        }
    ...
    case WM_SETFOCUS:
    {
        ...
        if (!CONSOLE_IS_TSF_ACTIVE() && !g_ciConsoleInformation.pInputBuffer->ImeMode.Disable)
        {
            if (ActivateTextServices(g_ciConsoleInformation.hWnd, GetImeSuggestionWindowPos))
            {
                g_ciConsoleInformation.Flags |= CONSOLE_TSF_ACTIVATED;
    ...
    case WM_IME_NOTIFY:
    {
        if (CONSOLE_IS_TSF_ACTIVE() && NotifyTextServices(Message, wParam, lParam, &Status))
        {
            break;
        }
        goto CallDefWin;
