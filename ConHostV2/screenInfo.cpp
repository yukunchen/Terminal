...
SCREEN_INFORMATION::SCREEN_INFORMATION(_In_ const CHAR_INFO ciFill, _In_ const CHAR_INFO ciPopupFill) :
    ...
    ResizingWindow(0),
    Next(nullptr),
    _wAttributes(ciFill.Attributes),
    _wPopupAttributes(ciPopupFill.Attributes),
    WheelDelta(0),
    HWheelDelta(0),
                                            ...
                                            _Outptr_ PPSCREEN_INFORMATION const ppScreen)
{
    *ppScreen = nullptr;
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        /*COORD const coordMin = pScreen->GetMinWindowSizeInCharacters(g_pfiFontInfo[nFont].Size);

        coordScreenBufferSize.X = max(coordScreenBufferSize.X, coordMin.X);
        coordWindowSize.X = max(coordWindowSize.X, coordMin.X);
*/
        pScreen->BufferViewport.Left = 0;
        pScreen->BufferViewport.Top = 0;
        pScreen->BufferViewport.Right = coordWindowSize.X - 1;
        pScreen->BufferViewport.Bottom = coordWindowSize.Y - 1;

    /*    COORD const coordMax = pScreen->GetMaxWindowSizeInCharacters(g_pfiFontInfo[nFont].Size);

        if (pScreen->BufferViewport.Right >= coordMax.X)
        {
            pScreen->BufferViewport.Right = coordMax.X - 1;
            coordWindowSize.X = pScreen->GetScreenWindowSizeX();
        }
        if (pScreen->BufferViewport.Bottom >= coordMax.Y)
        {
            pScreen->BufferViewport.Bottom = coordMax.Y - 1;
            coordWindowSize.Y = pScreen->GetScreenWindowSizeY();
        }*/
        pScreen->ScreenBufferSize = coordScreenBufferSize;
...
NTSTATUS SCREEN_INFORMATION::_InitializeOutputStateMachine()
{
    ...
    if (NT_SUCCESS(status))
    {
        ...
        if (!AdaptDispatch::CreateInstance(_pConApi, _pBufferWriter, _wAttributes, &_pAdapter))
        {
                                               ...
                                               _Out_writes_(COLOR_TABLE_SIZE) LPCOLORREF lpColorTable) const
{
    ...
    *pwAttributes = this->_wAttributes;
    *pwPopupAttributes = this->_wPopupAttributes;

    // the copy length must be constant for now to keep OACR happy with buffer overruns.
    CopyMemory(lpColorTable, g_ciConsoleInformation.GetColorTable(), COLOR_TABLE_SIZE * sizeof(COLORREF));
...
{
    return CalcWindowSizeX(&this->BufferViewport);
}

SHORT SCREEN_INFORMATION::GetScreenWindowSizeY() const
{
    return CalcWindowSizeY(&this->BufferViewport);
}
...
void SCREEN_INFORMATION::ResetTextFlags(_In_ short const sStartX, _In_ short const sStartY, _In_ short const sEndX, _In_ short const sEndY)
{
    ...
    if (this->IsActiveScreenBuffer())
    {
        ...
        if (sStartX == sEndX && sStartY == sEndY)
        {
            ...
            Row->AttrRow.FindAttrIndex(sStartX, &Attr, &CountOfAttr);
            NotifyWinEvent(EVENT_CONSOLE_UPDATE_SIMPLE, g_ciConsoleInformation.hWnd, MAKELONG(sStartX, sStartY), MAKELONG(Char, Attr->Attr));
        }
        else
        {
...
VOID SCREEN_INFORMATION::InternalUpdateScrollBars()
{
    ...
    si.nPos = this->BufferViewport.Top;
    ...
    si.nPos = this->BufferViewport.Left;
...
NTSTATUS SCREEN_INFORMATION::SetViewportOrigin(_In_ const BOOL fAbsolute, _In_ const COORD coordWindowOrigin)
{
    ...
    if (!fAbsolute)
    {
        ...
        NewWindow.Left = BufferViewport.Left + coordWindowOrigin.X;
        NewWindow.Top = BufferViewport.Top + coordWindowOrigin.Y;
    }
    else
    {
        if (coordWindowOrigin.X == BufferViewport.Left && coordWindowOrigin.Y == BufferViewport.Top)
        {
            return STATUS_SUCCESS;
        }
    ...
    else
    {
        ...
        BufferViewport = NewWindow;
        Tracing::s_TraceWindowViewport(&BufferViewport);
    }
...
NTSTATUS SCREEN_INFORMATION::SetViewportRect(_In_ SMALL_RECT* const prcNewViewport)
{
    ...
    if (memcmp(&BufferViewport, prcNewViewport, sizeof(SMALL_RECT)))
    {
        return STATUS_SUCCESS;
    }
...
void SCREEN_INFORMATION::ProcessResizeWindow(_In_ const RECT* const prcClientNew, _In_ const RECT* const prcClientOld)
{
    ...
    ASSERT(BufferViewport.Top >= 0);
    ASSERT(BufferViewport.Top < BufferViewport.Bottom);
    ASSERT(BufferViewport.Left < BufferViewport.Right);
}
...
void SCREEN_INFORMATION::_InternalSetViewportSize(_In_ const COORD* const pcoordSize, _In_ bool const fResizeFromTop, _In_ bool const fResizeFromLeft)
{
    ...
    if (fResizeFromLeft)
    {
        ...
        const SHORT sLeftProposed = (BufferViewport.Left - DeltaX);
        if (sLeftProposed >= 0)
        {
            // there's enough room in the backlog to just expand left
            this->BufferViewport.Left -= DeltaX;
        }
        else
        {
            ...
            this->BufferViewport.Left = 0;
            this->BufferViewport.Right += (SHORT)abs(sLeftProposed);
        }
    }
    else
    {
        // we're being horizontally sized from the right border
        const SHORT sRightProposed = (this->BufferViewport.Right + DeltaX);
        if (sRightProposed <= (this->ScreenBufferSize.X - 1))
        {
            this->BufferViewport.Right += DeltaX;
        }
        else
        {
            this->BufferViewport.Right = (this->ScreenBufferSize.X - 1);
            this->BufferViewport.Left -= (sRightProposed - (this->ScreenBufferSize.X - 1));
        }
    }

    if (fResizeFromTop)
    {
        const SHORT sTopProposed = (BufferViewport.Top - DeltaY);
        ...
        if (sTopProposed >= 0)
        {
            ...
            if (this->BufferViewport.Top > 0)
            {
                // there's enough room in the backlog to just expand the top
                this->BufferViewport.Top -= DeltaY;
            }
            else
            {
                ...
                ASSERT(DeltaY <= 0);
                this->BufferViewport.Bottom += DeltaY;
            }
        }
        else
        {
            ...
            this->BufferViewport.Top = 0;
            this->BufferViewport.Bottom += (SHORT)abs(sTopProposed);
        }
    }
    else
    {
        // we're being vertically sized from the bottom border
        const SHORT sBottomProposed = (BufferViewport.Bottom + DeltaY);
        if (sBottomProposed <= (this->ScreenBufferSize.Y - 1))
        {
            ...
            if (this->BufferViewport.Bottom + DeltaY < coordValidEnd.Y)
            {
                ...
                if (this->BufferViewport.Top < 0)
                {
                    ...
                    const short cRemainder = 0 - this->BufferViewport.Top;
                    BufferViewport.Top += cRemainder;
                    ASSERT(BufferViewport.Top == 0);
                    BufferViewport.Bottom += cRemainder;
                }
            }
            else
            {
        ...
        {
            this->BufferViewport.Bottom = (this->ScreenBufferSize.Y - 1);
            this->BufferViewport.Top -= (sBottomProposed - (this->ScreenBufferSize.Y - 1));
        }
    }

    // Ensure the viewport is valid.
    // We can't have a negative left or top.
    if (BufferViewport.Left < 0)
    {
        BufferViewport.Right -= BufferViewport.Left;
        BufferViewport.Left = 0;
    }

    if (BufferViewport.Top < 0)
    {
        BufferViewport.Bottom -= BufferViewport.Top;
        BufferViewport.Top = 0;
    }

    // Bottom and right cannot pass the final characters in the array.
    BufferViewport.Right = min(BufferViewport.Right, ScreenBufferSize.X - 1);
    BufferViewport.Bottom = min(BufferViewport.Bottom, ScreenBufferSize.Y - 1);

    Tracing::s_TraceWindowViewport(&this->BufferViewport);
}
...
bool SCREEN_INFORMATION::IsMaximizedX() const
{
    ...
    return BufferViewport.Left == 0 && (BufferViewport.Right + 1 == ScreenBufferSize.X);
}

bool SCREEN_INFORMATION::IsMaximizedY() const
{
    // If the viewport is displaying the entire size of the allocated buffer, it's maximized.
    return BufferViewport.Top == 0 && (BufferViewport.Bottom + 1 == ScreenBufferSize.Y);
}
...
NTSTATUS SCREEN_INFORMATION::ResizeWithReflow(_In_ COORD const coordNewScreenSize)
{
    ...
    CHAR_INFO ciFill;
    ciFill.Attributes = _wAttributes;
    ...
    SHORT const sCursorHeightInViewportBefore = TextInfo->GetCursor()->GetPosition().Y - BufferViewport.Top;

    if (NT_SUCCESS(status))
    {
        // skip any drawing updates that might occur as we manipulate the new buffer
        pNewBuffer->GetCursor()->StartDeferDrawing();

        // We need to save the delta between the final letter in the final row and the active cursor position.
        // This will be used later to ensure the cursor maintains the same offset from the final piece of text
        // after we have reflowed the buffer. (See cursor adjustments below.)
        int const iCursorDelta = Utils::s_CompareCoords(TextInfo->GetCursor()->GetPosition(), TextInfo->GetLastNonSpaceCharacter());

        short const cOldRowsTotal = TextInfo->GetCursor()->GetPosition().Y + 1;
        ...
        for (short iOldRow = 0; iOldRow < cOldRowsTotal; iOldRow++)
        {
            ...
            for (short iOldCol = 0; iOldCol < iRight; iOldCol++)
            {
                ...
                pRow->AttrRow.FindAttrIndex(iOldCol, &rgapAttr, &cAttrApplies);
                const WORD wAttr = rgapAttr->Attr;

                // Insert it into the new buffer
                pNewBuffer->InsertCharacter(wchChar, bKAttr, wAttr);
            }
            ...
            if (iRight < cOldColsTotal && !pRow->CharRow.WasWrapForced())
            {
                ...
                {
                    pNewBuffer->InsertNewline();
                }
            }
        }

        // Advance the cursor to the same offset as before
        // Subtract 1 from delta because the cursor is already "one after" the last piece of text after printing it back into the buffer.
        if (iCursorDelta > 0)
        {
            for (int iDelta = 0; iDelta < iCursorDelta - 1; iDelta++)
            {
                pNewBuffer->IncrementCursor();
            }
        }
        else
        {
        ...
        SHORT const sCursorHeightInViewportAfter = pNewBuffer->GetCursor()->GetPosition().Y - BufferViewport.Top;
        ...
        ULONG const ulSize = TextInfo->GetCursor()->GetSize();
        ...
        pNewBuffer->GetCursor()->SetSize(ulSize);
        pNewBuffer->GetCursor()->EndDeferDrawing();
    }
...
NTSTATUS SCREEN_INFORMATION::ResizeTraditional(_In_ COORD const coordNewScreenSize)
{
    ...
    DBCS_SCREEN_BUFFER* pNewDbcsScreenBuffer;
    PBYTE TextRowPtrA;
    ...
    if (!NT_SUCCESS(DBCS_SCREEN_BUFFER::CreateInstance(coordNewScreenSize, &pNewDbcsScreenBuffer)))
    {
    ...
    if (coordNewScreenSize.Y != this->ScreenBufferSize.Y)
    {
        ...
        Temp = (PROW)ConsoleHeapAlloc(SCREEN_TAG, cbRows);
        if (Temp == nullptr)
        {
            ConsoleHeapFree(TextRows);
            delete pNewDbcsScreenBuffer;
            return STATUS_NO_MEMORY;
        }
        ...
        CopyMemory(Temp, &pTextInfo->Rows[TopRowIndex], NumToCopy * sizeof(ROW));
        if (TopRowIndex != 0 && NumToCopy != coordNewScreenSize.Y)
        {
            ...
            CopyMemory(&Temp[NumToCopy], pTextInfo->Rows, NumToCopy2 * sizeof(ROW));
        }
        for (i = 0; i < LimitY; i++)
        {
        ...
        if (coordNewScreenSize.Y < this->ScreenBufferSize.Y)
        {
            i = (TopRowIndex + coordNewScreenSize.Y) % this->ScreenBufferSize.Y;
            for (j = coordNewScreenSize.Y; j < this->ScreenBufferSize.Y; j++)
            {
                if (pTextInfo->Rows[i].AttrRow.Length > 1)
                {
                    ConsoleHeapFree(pTextInfo->Rows[i].AttrRow.Attrs);
                }
                if (++i == this->ScreenBufferSize.Y)
                {
                    i = 0;
                }
            }
        ...
        else if (coordNewScreenSize.Y > this->ScreenBufferSize.Y)
        {
            for (i = this->ScreenBufferSize.Y; i < coordNewScreenSize.Y; i++)
            {
                ...
                Temp[i].AttrRow.AttrPair.Length = coordNewScreenSize.X;
                Temp[i].AttrRow.AttrPair.Attr = this->_wAttributes;
                Temp[i].AttrRow.Attrs = &Temp[i].AttrRow.AttrPair;
            }
        }
    ...
    TextRowPtrA = pNewDbcsScreenBuffer->KAttrRows;
    for (i = 0, TextRowPtr = TextRows; i < LimitY; i++, TextRowPtr += coordNewScreenSize.X)
    {
        CopyMemory(TextRowPtr, pTextInfo->Rows[i].CharRow.Chars, LimitX * sizeof(WCHAR));
        if (TextRowPtrA)
        {
            CopyMemory(TextRowPtrA, pTextInfo->Rows[i].CharRow.KAttrs, LimitX * sizeof(CHAR));
        }
    ...
    delete pTextInfo->pDbcsScreenBuffer;
    pTextInfo->pDbcsScreenBuffer = pNewDbcsScreenBuffer;
    ...
    if (coordNewScreenSize.X != this->ScreenBufferSize.X)
    {
        for (i = 0; i < LimitY; i++)
        {
            ...
            if (coordNewScreenSize.X > this->ScreenBufferSize.X)
            {
                pTextInfo->Rows[i].AttrRow.FindAttrIndex((SHORT)(this->ScreenBufferSize.X - 1), &IndexedAttr, &CountOfAttr);
                ASSERT(IndexedAttr <= &pTextInfo->Rows[i].AttrRow.Attrs[pTextInfo->Rows[i].AttrRow.Length - 1]);
                IndexedAttr->Length += coordNewScreenSize.X - this->ScreenBufferSize.X;
            }
            else
            {
                pTextInfo->Rows[i].AttrRow.FindAttrIndex((SHORT)(coordNewScreenSize.X - 1), &IndexedAttr, &CountOfAttr);

                SHORT sCountOfAttr;
                if (!SUCCEEDED(UIntToShort(CountOfAttr, &sCountOfAttr)))
                {
                    ASSERT(false);
                    return STATUS_UNSUCCESSFUL;
                }

                IndexedAttr->Length -= sCountOfAttr - 1;
                if (pTextInfo->Rows[i].AttrRow.Length != 1)
                {
                    pTextInfo->Rows[i].AttrRow.Length = (SHORT)(IndexedAttr - pTextInfo->Rows[i].AttrRow.Attrs + 1);
                    if (pTextInfo->Rows[i].AttrRow.Length != 1)
                    {
                        PATTR_PAIR AttrsTmp;

                        AttrsTmp = (PATTR_PAIR)ConsoleHeapReAlloc(0, pTextInfo->Rows[i].AttrRow.Attrs, pTextInfo->Rows[i].AttrRow.Length * sizeof(ATTR_PAIR));
                        if (AttrsTmp == nullptr)
                        {
                            ConsoleHeapFree(pTextInfo->Rows[i].AttrRow.Attrs);
                        }

                        pTextInfo->Rows[i].AttrRow.Attrs = AttrsTmp;
                    }
                    else
                    {
                        pTextInfo->Rows[i].AttrRow.AttrPair = *IndexedAttr;
                        ConsoleHeapFree(pTextInfo->Rows[i].AttrRow.Attrs);
                        pTextInfo->Rows[i].AttrRow.Attrs = &pTextInfo->Rows[i].AttrRow.AttrPair;
                    }
                }
            }
        }
...
void SCREEN_INFORMATION::MakeCursorVisible(_In_ const COORD CursorPosition)
{
    ...
    if (CursorPosition.X > this->BufferViewport.Right)
    {
        WindowOrigin.X = CursorPosition.X - this->BufferViewport.Right;
    }
    else if (CursorPosition.X < this->BufferViewport.Left)
    {
        WindowOrigin.X = CursorPosition.X - this->BufferViewport.Left;
    }
    else
    {
        WindowOrigin.X = 0;
    ...
    if (CursorPosition.Y > this->BufferViewport.Bottom)
    {
        WindowOrigin.Y = CursorPosition.Y - this->BufferViewport.Bottom;
    }
    else if (CursorPosition.Y < this->BufferViewport.Top)
    {
        WindowOrigin.Y = CursorPosition.Y - this->BufferViewport.Top;
    }
    else
    {
        WindowOrigin.Y = 0;
...
NTSTATUS SCREEN_INFORMATION::_CreateAltBuffer(_Out_ SCREEN_INFORMATION** const ppsiNewScreenBuffer)
{
    ...
    Fill.Attributes = this->_wAttributes;

    COORD WindowSize;
    WindowSize.X = (SHORT)CalcWindowSizeX(&BufferViewport);
    WindowSize.Y = (SHORT)CalcWindowSizeY(&BufferViewport);
...
WORD SCREEN_INFORMATION::GetAttributes() const
{
    return _wAttributes;
}
...
WORD SCREEN_INFORMATION::GetPopupAttributes() const
{
    return _wPopupAttributes;
}
...
// Return value:
// <none>
void SCREEN_INFORMATION::SetAttributes(_In_ const WORD wAttributes)
{
    _wAttributes = wAttributes;

    CHAR_INFO ciFill = TextInfo->GetFill();
    ciFill.Attributes = wAttributes;
...
// Return value:
// <none>
void SCREEN_INFORMATION::SetPopupAttributes(_In_ const WORD wPopupAttributes)
{
    _wPopupAttributes = wPopupAttributes;
}
