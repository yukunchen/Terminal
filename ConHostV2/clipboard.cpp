    ...
    if (NT_SUCCESS(status))
    {
        ...
        for (iRect = 0; iRect < cRectsSelected; iRect++)
        {
            ...
            if (NT_SUCCESS(status))
            {
                ...
                ReadRectFromScreenBuffer(pScreenInfo, coordSourcePoint, rgSelection, coordSelectionSize, &srTargetRect);
                ...
                if (NT_SUCCESS(status))
                {
                    ...
                    if (bMungeData)
                    {
                        PROW pRow = pScreenInfo->TextInfo->GetRowByOffset(iRow);
                    ...
                    // remeasure string
                    if (!SUCCEEDED(StringCchLengthW(pwszSelection, cSelectionLength, &cSelectionLength)))
                    {
                        status = STATUS_UNSUCCESSFUL;
                    }
                                                ...
                                                _In_reads_(cTotalRows) const size_t* const rgTempRowLengths)
{
    ...
    if (NT_SUCCESS(status))
    {
        ...
        for (UINT iRow = 0; iRow < cTotalRows; iRow++)
        {
            ...
            if (!SUCCEEDED(StringCchLengthW(rgTempRows[iRow], rgTempRowLengths[iRow] + 1, &cLength)))
            {
                status = STATUS_UNSUCCESSFUL;
                break;
            }
        ...
        if (NT_SUCCESS(status))
        {
            ...
            if (NT_SUCCESS(status))
            {
                ...
                if (NT_SUCCESS(status))
                {
                    ...
                    for (UINT iRow = 0; iRow < cTotalRows; iRow++)
                    {
                        if (!SUCCEEDED(StringCchCopyW(pwszClipboardPos, rgTempRowLengths[iRow] + 1, rgTempRows[iRow])))
                        {
                            status = STATUS_UNSUCCESSFUL;
                            break;
                        }
