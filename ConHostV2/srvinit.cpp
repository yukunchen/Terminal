                  ...
                  _In_ PCWSTR pwszAppName)
{
    ...
    if (pLinkSettings->GetStartupFlags() & STARTF_TITLEISLINKNAME)
    {
        if (SUCCEEDED(CoInitializeEx(nullptr, COINIT_APARTMENTTHREADED)))
        {
            ...
            {
                if (!SUCCEEDED(StringCbCopyNW(g_ciConsoleInformation.LinkTitle, cbTitle, pwszTitle, *pdwTitleLength)))
                {
    ...
    if (wszIconLocation[0] == L'\0')
    {
        ...
        // the shortcut)
        const DWORD dwLinkLen = SearchPathW(pwszCurrDir, pwszAppName, nullptr, sizeof(wszIconLocation), wszIconLocation, nullptr);
...
PWSTR TranslateConsoleTitle(_In_ PCWSTR pwszConsoleTitle, _In_ const BOOL fUnexpand, _In_ const BOOL fSubstitute)
{
    ...
    if (NT_SUCCESS(NT_TESTNULL(pwszSysRoot)))
    {
...
#pragma prefast(suppress:26018, "We've guaranteed that cchSystemRoot is equal to or smaller than cchConsoleTitle in size.")
                ...
                CopyMemory(TranslatedConsoleTitle, SYSTEM_ROOT, cbSystemRoot);
...
NTSTATUS ConsoleInitializeConnectInfo(_In_ PCONSOLE_API_MSG Message, _Out_ PCONSOLE_API_CONNECTINFO Cac)
{
    ...
    CopyMemory(Cac->AppName, Data.ApplicationName, sizeof(Cac->AppName));
    CopyMemory(Cac->Title, Data.Title, sizeof(Cac->Title));
    CopyMemory(Cac->CurDir, Data.CurrentDirectory, sizeof(Cac->CurDir));
