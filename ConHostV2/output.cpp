...
VOID SetConsoleWindowOwner(_In_ HWND hwnd, _Inout_opt_ PCONSOLE_PROCESS_HANDLE pProcessData)
{
    ...
    HANDLE ProcessId;
    HANDLE ThreadId;
    if (ARGUMENT_PRESENT(pProcessData))
    {
...
// Arguments:
// - prgSrcAttrPairs - pointer to source attribute string
// - cSrcAttrPairs - length of source.  for example, the length of { 4, BLUE } is 1.
// - prgMergeAttrPairs - pointer to attribute string to insert into source
// - cMergeAttrPairs - length of merge
// - pprgTargetAttrPairs - where to store pointer to resulting attribute string
// - cOutAttrPairs - where to store length of resulting attribute string
// - sStartIndex - index into Source at which to insert Merge String.
// - sEndIndex - index into Source at which to stop insertion of Merge String
// Return Value:
// - <none>
                          ...
                          _In_ const SCREEN_INFORMATION * const pScreenInfo)
{
    ...
    if (cMergeAttrPairs == 1 && pRow->AttrRow.Length == 1)
    {
        if (pRow->AttrRow.Attrs->Attr == prgMergeAttrPairs->Attr)
        {
            *cOutAttrPairs = 1;
            *pprgTargetAttrPairs = &pRow->AttrRow.AttrPair;
            return STATUS_SUCCESS;
        }
        if (sStartIndex == 0 && sEndIndex == (SHORT)(pScreenInfo->ScreenBufferSize.X - 1))
        {
            PATTR_PAIR const NewString = &pRow->AttrRow.AttrPair;
            NewString->Attr = prgMergeAttrPairs->Attr;
            *cOutAttrPairs = 1;
            *pprgTargetAttrPairs = NewString;
            return STATUS_SUCCESS;
        }
    }

    PATTR_PAIR const NewString = (PATTR_PAIR) ConsoleHeapAlloc(SCREEN_TAG, (cSrcAttrPairs + cMergeAttrPairs + 1) * sizeof(ATTR_PAIR));
    ...
    PATTR_PAIR SrcAttr = prgSrcAttrPairs;
    PATTR_PAIR const SrcEnd = prgSrcAttrPairs + cSrcAttrPairs;
    PATTR_PAIR TargetAttr = NewString;
    SHORT i = 0;
    if (sStartIndex > 0)
    {
        while (i < sStartIndex)
        ...
        // counter, back to the sStartIndex.  i will be larger than the
        // sStartIndex in the case where the last attribute pair copied had
        // a length greater than the number needed to reach sStartIndex.

        TargetAttr--;
        if (i > sStartIndex)
        {
            TargetAttr->Length -= i - sStartIndex;
        }
        if (prgMergeAttrPairs->Attr == TargetAttr->Attr)
        {
            TargetAttr->Length += prgMergeAttrPairs->Length;
            cMergeAttrPairs -= 1;
            prgMergeAttrPairs++;
        }
        TargetAttr++;
    ...
    CopyMemory(TargetAttr, prgMergeAttrPairs, cMergeAttrPairs * sizeof(ATTR_PAIR));
    TargetAttr += cMergeAttrPairs;
    ...
    if (SrcAttr != SrcEnd || i != (SHORT)(sEndIndex + 1))
    {
        // see if we've gone past the right attribute.  if so, back up and copy the attribute and the correct length.
        #pragma prefast(push)
        #pragma prefast(disable:26001, "SrcAttr and TargetAttr are bounded.")
        #pragma prefast(disable:26019, "SrcAttr and TargetAttr are bounded.")
        TargetAttr--;
        if (i > (SHORT)(sEndIndex + 1))
        {
            SrcAttr--;
            if (TargetAttr->Attr == SrcAttr->Attr)
            {
                TargetAttr->Length += i - (sEndIndex + 1);
            }
            else
            {
                TargetAttr++;
                TargetAttr->Attr = SrcAttr->Attr;
                TargetAttr->Length = (SHORT)(i - (sEndIndex + 1));
            }
            SrcAttr++;
        }
        #pragma prefast(pop)
        ...
        if (SrcAttr < SrcEnd)
        {
            CopyMemory(TargetAttr, SrcAttr, (SrcEnd - SrcAttr) * sizeof(ATTR_PAIR));
            TargetAttr += SrcEnd - SrcAttr;
        }
    }

    *cOutAttrPairs = (WORD) (TargetAttr - NewString);
    *pprgTargetAttrPairs = NewString;
    if (*cOutAttrPairs == 1)
    {
        *pprgTargetAttrPairs = &pRow->AttrRow.AttrPair;
...
// Return Value:
// - <none>
void ReadRectFromScreenBuffer(_In_ const SCREEN_INFORMATION * const pScreenInfo,
                              ...
                              _In_ PSMALL_RECT const psrTargetRect)
{
    ...
    PROW pRow = pScreenInfo->TextInfo->GetRowByOffset(coordSourcePoint.Y);
    for (short iRow = 0; iRow < sYSize; iRow++)
    {
        ...
        PATTR_PAIR pAttr;
        UINT uiAttrTotal;
        pRow->AttrRow.FindAttrIndex(coordSourcePoint.X, &pAttr, &uiAttrTotal);

        short cAttrUsed = 0;
        short cAttrTotal = 0;

        // attempt safe cast. bail early if failed.
        if (!SUCCEEDED(UIntToShort(uiAttrTotal, &cAttrTotal)))
        {
            ASSERT(false);
            return;
        }
        ...
        for (short iCol = 0; iCol < sXSize; pciTargetPtr++)
        {
            ...
            pciTargetPtr->Attributes |= pAttr->Attr;
            iCol += 1;
            if (++cAttrUsed == cAttrTotal && iCol < sXSize)
            {
                pAttr++;
                cAttrUsed = 0;
                cAttrTotal = pAttr->Length;
            }
        }
...
// Return Value:
// - <none>
void _CopyRectangle(_In_ PSCREEN_INFORMATION pScreenInfo, _In_ const SMALL_RECT * const psrSource, _In_ const COORD coordTarget)
{
    ...
    if (SUCCEEDED(NT_TESTNULL(rgciScrollBuffer)))
    {
        ReadRectFromScreenBuffer(pScreenInfo, SourcePoint, rgciScrollBuffer, Size, &Target);

        WriteRectToScreenBuffer((PBYTE)rgciScrollBuffer, Size, &Target, pScreenInfo, coordTarget);  // ScrollBuffer won't need conversion
...
NTSTATUS ReadScreenBuffer(_In_ const SCREEN_INFORMATION * const pScreenInfo, _Inout_ PCHAR_INFO pciBuffer, _Inout_ PSMALL_RECT psrReadRegion)
{
    ...
    ReadRectFromScreenBuffer(pScreenInfo, SourcePoint, pciBuffer, TargetSize, &Target);
    return STATUS_SUCCESS;
}
...
NTSTATUS WriteScreenBuffer(_In_ PSCREEN_INFORMATION pScreenInfo, _In_ PCHAR_INFO pciBuffer, _Inout_ PSMALL_RECT psrWriteRegion)
{
    ...
    WriteRectToScreenBuffer((PBYTE) pciBuffer, SourceSize, &SourceRect, pScreenInfo, TargetPoint);
    return STATUS_SUCCESS;
}
                          ...
                          _Inout_ PULONG pcRecords)
{
    ...
    if (ulStringType == CONSOLE_ASCII)
    {
        ...
        if (!SUCCEEDED(ULongMult(*pcRecords, sizeof(WCHAR), &AllocationSize)))
        {
            return STATUS_INVALID_PARAMETER;
        }
...

    {
        SHORT CountOfAttr;
        PATTR_PAIR Attr;
        PROW Row = pScreenInfo->TextInfo->GetRowByOffset(coordRead.Y);
        ASSERT(Row != nullptr);
        PWCHAR Char;
        SHORT j, k;
        PBYTE AttrP = nullptr;
        ...
        if ((ulStringType == CONSOLE_ASCII) || (ulStringType == CONSOLE_REAL_UNICODE) || (ulStringType == CONSOLE_FALSE_UNICODE))
        {
            while (NumRead < *pcRecords)
            {
                ...
                {
                    CopyMemory(BufPtr, Char, (*pcRecords - NumRead) * sizeof(WCHAR));
                    CopyMemory(BufPtrA, AttrP, (*pcRecords - NumRead) * sizeof(CHAR));

                    NumRead += *pcRecords - NumRead;
                    break;
                }

                CopyMemory(BufPtr, Char, (pScreenInfo->ScreenBufferSize.X - X) * sizeof(WCHAR));
                BufPtr = (PWCHAR)((PBYTE)BufPtr + ((pScreenInfo->ScreenBufferSize.X - X) * sizeof(WCHAR)));

                CopyMemory(BufPtrA, AttrP, (pScreenInfo->ScreenBufferSize.X - X) * sizeof(CHAR));
...
#pragma prefast(suppress:__WARNING_BUFFER_OVERFLOW, "This code is fine")
        ...
        else if (ulStringType == CONSOLE_ATTRIBUTE)
        {
            ...
            while (NumRead < *pcRecords)
            {
                ...
                UINT uiCountOfAttr;
                Row->AttrRow.FindAttrIndex(X, &Attr, &uiCountOfAttr);

                // attempt safe cast. bail early if failed.
                if (!SUCCEEDED(UIntToShort(uiCountOfAttr, &CountOfAttr)))
                {
                    ASSERT(false);
                    return STATUS_UNSUCCESSFUL;
                }

                CountOfAttr = (short)uiCountOfAttr;
                ...
                for (j = X; j < pScreenInfo->ScreenBufferSize.X; TargetPtr++)
                {
                    ...
                    else if (*AttrP & CHAR_ROW::ATTR_LEADING_BYTE)
                    {
                        ...
                        {
                            *TargetPtr = Attr->Attr | (WCHAR)(*AttrP & CHAR_ROW::ATTR_DBCSSBCS_BYTE) << 8;
                        }
                    }
                    else
                    {
...
// Return Value:
void StreamScrollRegion(_Inout_ PSCREEN_INFORMATION pScreenInfo)
{
    // Rotate the circular buffer around and wipe out the previous final line.
    pScreenInfo->TextInfo->IncrementCircularBuffer();
                      ...
                      _In_ CHAR_INFO ciFill)
{
    ...
    if (!(TargetRectangle.Bottom < TargetRectangle.Top || TargetRectangle.Right < TargetRectangle.Left))
    {
            ...
            ScrollRectangle2.Left == TargetRectangle.Left && ScrollRectangle2.Top > TargetRectangle.Top && ScrollRectangle2.Top < TargetRectangle.Bottom)
        {
            ...
            {
                _CopyRectangle(pScreenInfo, &ScrollRectangle2, TargetPoint);
            }
        ...
        else
        {
            ...
            if (SUCCEEDED(NT_TESTNULL(rgciScrollBuffer)))
            {
                ...
                ReadRectFromScreenBuffer(pScreenInfo, SourcePoint, rgciScrollBuffer, Size, &TargetRect);
                ...
                WriteRectToScreenBuffer((PBYTE)rgciScrollBuffer, Size, &SourceRectangle, pScreenInfo, TargetPoint);
...
NTSTATUS SetActiveScreenBuffer(_Inout_ PSCREEN_INFORMATION pScreenInfo)
{
    ...
    // Empty input buffer.
    FlushAllButKeys();
    SetScreenColors(pScreenInfo, pScreenInfo->GetAttributes(), pScreenInfo->GetPopupAttributes(), FALSE);
...
// Checks if the active language matches the output charset.
bool IsIMEInputSupported()
{
    return true;
}

// Checks if the status line needs to be reserved for IME conversions
bool IsConvAreaEnabled()
{
    return CONSOLE_IS_TSF_ACTIVE() && !g_ciConsoleInformation.pInputBuffer->ImeMode.Disable;
}
