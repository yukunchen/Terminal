...
retry_clean:
        CopyMemory(gaKeyDef, gaDefaultKeyDef, sizeof gaKeyDef);
        return;
    }
    ...
    // Copy the entity
    CopyMemory(gaKeyDef, pKeyDefBuf->table, sizeof gaKeyDef);
}
                                ...
                                _In_ BOOLEAN UnicodeExe)
{
    ...
    if (UnicodeExe)
    {
        ...
        CopyMemory(AliasList->ExeName, ExeName, ExeLength);
                        ...
                        _In_ BOOLEAN UnicodeExe)
{
    ...
    while (ListNext != ListHead)
    {
        ...
        if (AliasList->ExeLength == ExeLength && !_wcsnicmp(AliasList->ExeName, UnicodeExeName, ExeLength))
        {
            if (!UnicodeExe)
            {
...
PALIAS FindAlias(_In_ PEXE_ALIAS_LIST AliasList, _In_reads_bytes_(AliasLength) const WCHAR *AliasName, _In_ USHORT AliasLength)
{
    ...
    while (ListNext != ListHead)
    {
        ...
        if (Alias->SourceLength == AliasLength && !_wcsnicmp(Alias->Source, AliasName, AliasLength))
        {
                  ...
                  _In_ USHORT TargetLength)
{
    ...
    CopyMemory(Alias->Source, Source, SourceLength);
    CopyMemory(Alias->Target, Target, TargetLength);
    ...
    if (ExeAliasList)
    {
        ...
        if (Alias)
        {
            ...
            else
            {
                ...
                CopyMemory(Target, Alias->Target, Alias->TargetLength);
    ...
    if (ExeAliasList)
    {
        ...
        while (ListNext != ListHead)
        {
            ...
            if (a->Unicode)
            {
                ...
                {
                    CopyMemory(AliasesBufferPtrW, Alias->Source, Alias->SourceLength);
                    AliasesBufferPtrW += Alias->SourceLength / sizeof(WCHAR);
                    *AliasesBufferPtrW++ = (WCHAR)'=';
                    CopyMemory(AliasesBufferPtrW, Alias->Target, Alias->TargetLength);
...
NTSTATUS SrvGetConsoleAliasExes(_Inout_ PCONSOLE_API_MSG m, _In_opt_ PBOOL const /*ReplyPending*/)
{
    ...
    while (ListNext != ListHead)
    {
        ...
        if (a->Unicode)
        {
            ...
            {
                CopyMemory(AliasExesBufferPtrW, AliasList->ExeName, AliasList->ExeLength);
    ...
    *pcLines = 1;
        ...
        if (*TargetAlias == (WCHAR)'$' && (USHORT) (i + 1) < (USHORT) (Alias->TargetLength / sizeof(WCHAR)))
        {
            ...
            if (*TargetAlias >= (WCHAR)'1' && *TargetAlias <= (WCHAR)'9')
            {
                ...
                if (ArgNumber < NumSourceArgs)
                {
                    ...
                    {
                        CopyMemory(Buffer, Args[ArgNumber], ArgsLength[ArgNumber]);
            ...
            else if (*TargetAlias == (WCHAR)'*')
            {
                ...
                if (NumSourceArgs)
                {
                    ...
                    {
                        CopyMemory(Buffer, Args[0], SourceRemainderLength * sizeof(WCHAR));
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        *Buffer++ = UNICODE_LINEFEED;
        ...
        CopyMemory(pwchTarget, TmpBuffer, NewTargetLength);
    }
    ...
    if (CommandHistory)
    {
        for (SHORT i = 0; i < CommandHistory->NumberOfCommands; i++)
        {
            if (a->Unicode)
            {
                ...
                {
                    CopyMemory(CommandBufferW, CommandHistory->Commands[i]->Command, CommandHistory->Commands[i]->CommandLength);
...
PCOMMAND_HISTORY FindExeCommandHistory(_In_reads_(AppNameLength) PVOID AppName, _In_ DWORD AppNameLength, _In_ BOOLEAN const Unicode)
{
    ...
    while (ListNext != ListHead)
    {
        ...
        if (History->Flags & CLE_ALLOCATED && !_wcsnicmp(History->AppName, AppNamePtr, (USHORT) AppNameLength))
        {
            if (!Unicode)
            {
...
PCOMMAND_HISTORY AllocateCommandHistory(_In_reads_bytes_(cbAppName) PCWSTR pwszAppName, _In_ const DWORD cbAppName, _In_ HANDLE hProcess)
{
    ...
    while (ListNext != ListHead)
    {
        ...
        if ((History->Flags & CLE_ALLOCATED) == 0)
        {
            ...
            if (History->AppName && !_wcsnicmp(History->AppName, pwszAppName, (USHORT) cbAppName))
            {
    ...
    if (!SameApp && g_ciConsoleInformation.NumCommandHistories < g_ciConsoleInformation.GetNumberOfHistoryBuffers())
    {
        ...
        if (!SUCCEEDED(SizeTMult(g_ciConsoleInformation.GetHistoryBufferSize(), sizeof(PCOMMAND), &Size)))
        {
            return nullptr;
        }

        if (!SUCCEEDED(SizeTAdd(sizeof(COMMAND_HISTORY), Size, &TotalSize)))
        {
            return nullptr;
        }
        ...
        CopyMemory(History->AppName, pwszAppName, cbAppName);
...
NTSTATUS BeginPopup(_In_ PSCREEN_INFORMATION ScreenInfo, _In_ PCOMMAND_HISTORY CommandHistory, _In_ COORD PopupSize)
{
    ...
    Popup->Attributes = ScreenInfo->GetPopupAttributes();
                            ...
                            PWCHAR Buffer, _In_ ULONG BufferSize, _Out_ PULONG CommandSize)
{
    ...
    CopyMemory(Buffer, CommandRecord->Command, *CommandSize);
    return STATUS_SUCCESS;
}
    ...
    for (;;)
    {
        ...
        else if (Char == UNICODE_CARRIAGERETURN)
        {
            ...
            if (pCookedReadData->BytesRead > pCookedReadData->UserBufferSize || LineCount > 1)
            {
                ...
                CopyMemory(pCookedReadData->UserBuffer, pCookedReadData->BackupLimit, a->NumBytes);
            }
            else
            {
            ...
            if (!a->Unicode)
            {
                ...
                a->NumBytes = (ULONG) ConvertToOem(g_ciConsoleInformation.CP,
                                                   ...
                                                   TransBuffer,
                                                   a->NumBytes);
                CopyMemory(pCookedReadData->UserBuffer, TransBuffer, a->NumBytes);
...
NTSTATUS ProcessCopyFromCharInput(_In_ PCOOKED_READ_DATA const pCookedReadData, _In_ PCONSOLE_API_MSG WaitReplyMessage, _In_ BOOLEAN WaitRoutine)
{
    ...
    for (;;)
    {
        ...
        if (i != (int)(pCookedReadData->BytesRead / sizeof(WCHAR) + 1))
        {
            ...
            // Delete chars.
            CopyMemory(&pCookedReadData->BackupLimit[pCookedReadData->CurrentPosition],
    ...
    for (;;)
    {
        ...
        if (LastCommand)
        {
                ...
                (USHORT) (LastCommand->CommandLength / sizeof(WCHAR)) > (USHORT) pCookedReadData->CurrentPosition)
            {
                ...
                ASSERT(j > 0);
                CopyMemory(pCookedReadData->BufPtr, &LastCommand->Command[pCookedReadData->CurrentPosition], j * sizeof(WCHAR));
    ...
    for (;;)
    {
        ...
        if (Char >= (WCHAR)0x30 && Char <= (WCHAR)0x39)
        {
            if (Popup->NumberRead < 5)
            {
                ...
                RealAttributes = pCookedReadData->ScreenInfo->GetAttributes();
                ...
                Status = WriteCharsLegacy(pCookedReadData->ScreenInfo,
                                          ...
                                          nullptr);
                ASSERT(NT_SUCCESS(Status));
                pCookedReadData->ScreenInfo->SetAttributes(RealAttributes);
        ...
        switch (wch)
        {
            ...
            case VK_HOME:
                if (dwKeyState & (RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED))
                {
                    ...
                    pCookedReadData->BytesRead -= pCookedReadData->CurrentPosition * sizeof(WCHAR);
                    pCookedReadData->CurrentPosition = 0;
                    CopyMemory(pCookedReadData->BackupLimit, pCookedReadData->BufPtr, pCookedReadData->BytesRead);
                    pCookedReadData->BufPtr = pCookedReadData->BackupLimit;
                    if (pCookedReadData->Echo)
                    {
                        Status = WriteCharsLegacy(pCookedReadData->ScreenInfo,
...
#pragma prefast(suppress:__WARNING_POTENTIAL_BUFFER_OVERFLOW_HIGH_PRIORITY, "This is fine")
                        CopyMemory(pCookedReadData->BufPtr, &LastCommand->Command[pCookedReadData->CurrentPosition], cchCount * sizeof(WCHAR));
...
#pragma prefast(suppress:__WARNING_BUFFER_OVERFLOW, "Not sure why prefast is getting confused here")
                    ...
                    // Delete char.
                    pCookedReadData->BytesRead -= sizeof(WCHAR);
                    CopyMemory(pCookedReadData->BufPtr,
                               pCookedReadData->BufPtr + 1,
                               pCookedReadData->BytesRead - (pCookedReadData->CurrentPosition * sizeof(WCHAR)));
...
PCOMMAND RemoveCommand(_In_ PCOMMAND_HISTORY CommandHistory, _In_ SHORT iDel)
{
    ...
    if (iDel < iLast)
    {
        CopyMemory(ppcDel, ppcDel + 1, (iLast - iDel) * sizeof(PCOMMAND));
    ...
    {
        MoveMemory(ppcFirst + 1, ppcFirst, (iDel - iFirst) * sizeof(PCOMMAND));
                          ...
                          _In_ DWORD Flags)
{
    ...
    for (SHORT i = 0; i < CommandHistory->NumberOfCommands; i++)
    {
        ...
        {
            if (!wcsncmp(pcmdT->Command, pwchIn, (USHORT) cbIn))
            {
                return CommandIndex;
            }
        }
                     ...
                     _In_ PSCREEN_INFORMATION ScreenInfo)
{
    ...
    // inverted attributes
    WORD const Attributes = (WORD) (((Popup->Attributes << 4) & 0xf0) | ((Popup->Attributes >> 4) & 0x0f));
                          ...
                          _In_ PSCREEN_INFORMATION const ScreenInfo)
{
    ...
    for (; i <= Popup->BottomIndex; i++)
    {
        ...
        if (!SUCCEEDED(StringCchLengthA(CommandNumberPtr, ARRAYSIZE(CommandNumber), &CommandNumberLength)))
        {
            return;
        }
        ...
        WriteCoord.X = (SHORT)(WriteCoord.X + CommandNumberLength);
        {
            ...
            CopyMemory(TransBuffer, CommandHistory->Commands[COMMAND_NUM_TO_INDEX(i, CommandHistory)]->Command, lStringLength * sizeof(WCHAR));
        ...
        memcmp(pCmdHistory->Commands[pCmdHistory->LastAdded]->Command, pwchCommand, cbCommand))
    {
        ...
        else
        {
            *ppCmd = (PCOMMAND) ConsoleHeapAlloc(HISTORY_TAG, cbCommand + sizeof(COMMAND));
            ...
            CopyMemory((*ppCmd)->Command, pwchCommand, cbCommand);
        }
    }
...
NTSTATUS DoSrvSetConsoleTitle(_In_ PVOID const Buffer, _In_ ULONG const cbOriginalLength, _In_ BOOLEAN const fUnicode)
{
    ...
    if (fUnicode)
    {
        if (!SUCCEEDED(ULongAdd(cbOriginalLength, sizeof(WCHAR), &cbTitleLength)))
        {
            Status = STATUS_UNSUCCESSFUL;
        }
    }
    ...
    {
        if (!SUCCEEDED(ULongMult(cbOriginalLength, sizeof(WCHAR), &cbTitleLength)))
        {
            Status = STATUS_UNSUCCESSFUL;
        }
        ...
        if (NT_SUCCESS(Status))
        {
            if (!SUCCEEDED(ULongAdd(cbTitleLength, sizeof(WCHAR), &cbTitleLength)))
            {
                Status = STATUS_UNSUCCESSFUL;
            }
        }
    ...
    if (NT_SUCCESS(Status)){
        ...
        if (NewTitle != nullptr)
        {
            ...
            else
            {
                ...
                {
                    CopyMemory(NewTitle, Buffer, cbOriginalLength);
