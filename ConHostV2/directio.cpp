                          ...
                          _Inout_opt_ PINPUT_RECORD DbcsLeadInpRec)
{
    ...
    ULONG NumBytes;
    if (!SUCCEEDED(DWordMult(NumRecords, sizeof(INPUT_RECORD), &NumBytes)))
    {
        return 0;
    }
    ...
    CopyMemory(TmpInpRec, InputRecords, NumBytes);
    BYTE AsciiDbcs[2];
    AsciiDbcs[1] = 0;
    ULONG i, j;
    ...
    if (!a->Unicode)
    {
        if (GetInputBufferFromHandle(HandleData)->ReadConInpDbcsLeadByte.Event.KeyEvent.uChar.AsciiChar)
        {
            if (a->NumRecords == 1)
            {
                ...
                *Buffer = GetInputBufferFromHandle(HandleData)->ReadConInpDbcsLeadByte;
...
#pragma prefast(suppress: 28132, "sizeof(DirectReadData) is used as an indicator of the recursion depth and needs to be the size of the pointer in this case. This is clearer as-is than measuring the pointer type.")
    ...
    __finally
    {
        ...
        if (Status != CONSOLE_STATUS_WAIT)
        {
            ...
            {
                if (fAddDbcsLead && GetInputBufferFromHandle(HandleData)->ReadConInpDbcsLeadByte.Event.KeyEvent.uChar.AsciiChar)
                {
                    a->NumRecords--;
                }
                ...
                a->NumRecords = TranslateInputToOem(Buffer,
                                                    a->NumRecords,
                                                    g_ciConsoleInformation.ReadConInpNumBytesUnicode,
                                                    a->Flags & CONSOLE_READ_NOREMOVE ? nullptr : &GetInputBufferFromHandle(HandleData)->ReadConInpDbcsLeadByte);
                if (fAddDbcsLead && GetInputBufferFromHandle(HandleData)->ReadConInpDbcsLeadByte.Event.KeyEvent.uChar.AsciiChar)
                {
                    *(Buffer - 1) = GetInputBufferFromHandle(HandleData)->ReadConInpDbcsLeadByte;
                    if (!(a->Flags & CONSOLE_READ_NOREMOVE))
                        ZeroMemory(&GetInputBufferFromHandle(HandleData)->ReadConInpDbcsLeadByte, sizeof(INPUT_RECORD));
                    a->NumRecords++;
                    Buffer--;
...
NTSTATUS SrvGetConsoleInput(_Inout_ PCONSOLE_API_MSG m, _Inout_ PBOOL ReplyPending)
{
    ...
    else
    {
        ...
        Status = ReadInputBuffer(GetInputBufferFromHandle(HandleData),
                                 Buffer,
                                 nLength,
                                 !!(a->Flags & CONSOLE_READ_NOREMOVE),
        ...
        else if (!a->Unicode)
        {
            a->NumRecords = TranslateInputToOem(Buffer,
                                                ...
                                                a->NumRecords - 1 :
                                                a->NumRecords,
                                                nBytesUnicode, a->Flags & CONSOLE_READ_NOREMOVE ? nullptr : &GetInputBufferFromHandle(HandleData)->ReadConInpDbcsLeadByte);
            if (fAddDbcsLead)
            {
                a->NumRecords++;
                Buffer--;
...
NTSTATUS SrvWriteConsoleInput(_Inout_ PCONSOLE_API_MSG m, _Inout_ PBOOL /*ReplyPending*/)
{
    ...
    {
        Status = DoSrvWriteConsoleInput(GetInputBufferFromHandle(HandleData), a, Buffer);
    }
...
NTSTATUS TranslateOutputToOem(_Inout_ PCHAR_INFO OutputBuffer, _In_ COORD Size)
{
    ...
    if (!SUCCEEDED(ShortToULong(Size.X, &uX)) || 
        !SUCCEEDED(ShortToULong(Size.Y, &uY)) || 
        !SUCCEEDED(ULongMult(uX, uY, &NumBytes)) || 
        !SUCCEEDED(ULongMult(NumBytes, 2 * sizeof(CHAR_INFO), &NumBytes)))
    {
    ...
    CopyMemory(TmpBuffer, OutputBuffer, Size.X * Size.Y * sizeof(CHAR_INFO));
...
NTSTATUS SrvWriteConsoleOutput(_Inout_ PCONSOLE_API_MSG m, _Inout_ PBOOL /*ReplyPending*/)
{
    ...
    else
    {
        ...
        BufferSize.X = (SHORT)(a->CharRegion.Right - a->CharRegion.Left + 1);
        BufferSize.Y = (SHORT)(a->CharRegion.Bottom - a->CharRegion.Top + 1);
        if (!SUCCEEDED(ULongMult(BufferSize.X, BufferSize.Y, &NumBytes)) || 
            !SUCCEEDED(ULongMult(NumBytes, sizeof(*Buffer), &NumBytes)) || 
            (Size < NumBytes))
        {
...
NTSTATUS SrvReadConsoleOutputString(_Inout_ PCONSOLE_API_MSG m, _Inout_ PBOOL /*ReplyPending*/)
{
    ...
    else
    {
        ...
        Status = ReadOutputString(GetScreenBufferFromHandle(HandleData), Buffer, a->ReadCoord, a->StringType, &a->NumRecords);
        if (NT_SUCCESS(Status))
        {
...
NTSTATUS SrvWriteConsoleOutputString(_Inout_ PCONSOLE_API_MSG m, _Inout_ PBOOL /*ReplyPending*/)
{
    ...
    else
    {
        ...
        else
        {
            ...
            Status = WriteOutputString(GetScreenBufferFromHandle(HandleData), Buffer, a->WriteCoord, a->StringType, &a->NumRecords, nullptr);
        }
    }
...
NTSTATUS SrvFillConsoleOutput(_Inout_ PCONSOLE_API_MSG m, _Inout_ PBOOL /*ReplyPending*/)
{
    ...
    {
        Status = DoSrvFillConsoleOutput(GetScreenBufferFromHandle(HandleData), a);
    }
                                   ...
                                   _In_ PCONSOLE_CREATESCREENBUFFER_MSG a)
{
    ...
    Fill.Attributes = psiExisting->GetAttributes();
