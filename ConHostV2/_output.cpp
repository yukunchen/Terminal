                               ...
                               _In_ const bool fWasLineWrapped)
{
    ...
    // copy chars
    CopyMemory(&Row->CharRow.KAttrs[TargetPoint.X], pchBuffer, cchBuffer * sizeof(CHAR));

    CopyMemory(&Row->CharRow.Chars[TargetPoint.X], pwchBuffer, cchBuffer * sizeof(WCHAR));
    ...
    if (Row->AttrRow.Length != 1 || Row->AttrRow.Attrs->Attr != pScreenInfo->GetAttributes())
    {
        PATTR_PAIR NewAttrs;
        WORD NewAttrsLength;
        ATTR_PAIR Attrs;

        Attrs.Length = cchBuffer;
        Attrs.Attr = pScreenInfo->GetAttributes();
...
// Return Value:
// - <none>
void WriteRectToScreenBuffer(_In_reads_(coordSrcDimensions.X * coordSrcDimensions.Y * sizeof(CHAR_INFO)) PBYTE const prgbSrc,
                             ...
                             _In_ const COORD coordDest)
{
    ...
    if (XSize > 80)
    {
        AttrBuf = (PATTR_PAIR) ConsoleHeapAlloc(TMP_TAG, XSize * sizeof(ATTR_PAIR));
        if (AttrBuf == nullptr)
        {
            return;
        }
    ...
    for (SHORT i = 0; i < YSize; i++)
    {
        ...
        if (AttrLength != Row->AttrRow.Length || memcmp(Row->AttrRow.Attrs, AttrBuf, AttrLength * sizeof(*Attr)))
        {
            PATTR_PAIR NewAttrs;
            WORD NewAttrsLength;
...
#pragma prefast(suppress:__WARNING_BUFFER_OVERFLOW, "ScreenBufferSize limits this, but it's very obtuse.")
                ...
                CopyMemory(AttrP, BufferA, (*pcRecords - NumWritten) * sizeof(CHAR));
#pragma prefast(suppress:__WARNING_BUFFER_OVERFLOW, "Code appears to be fine.")
                CopyMemory(Char, pvBuffer, (*pcRecords - NumWritten) * sizeof(WCHAR));
            ...
            else
            {
                ...
                CopyMemory(AttrP, BufferA, (pScreenInfo->ScreenBufferSize.X - X) * sizeof(CHAR));
                BufferA = (PBYTE)((PBYTE)BufferA + ((pScreenInfo->ScreenBufferSize.X - X) * sizeof(CHAR)));
                CopyMemory(Char, pvBuffer, (pScreenInfo->ScreenBufferSize.X - X) * sizeof(WCHAR));
    ...
    else if (ulStringType == CONSOLE_ATTRIBUTE)
    {
        ...
        AttrBuf = Attrs;
        if (pScreenInfo->ScreenBufferSize.X > 80)
        {
            AttrBuf = (PATTR_PAIR) ConsoleHeapAlloc(TMP_TAG, pScreenInfo->ScreenBufferSize.X * sizeof(ATTR_PAIR));
            if (AttrBuf == nullptr)
                return STATUS_NO_MEMORY;
        }
        {
                    ...
                    _Inout_ PULONG pcElements)  // this value is valid even for error cases
{
    ...
    else if (ulElementType == CONSOLE_ATTRIBUTE)
    {
        ...
        for (;;)
        {
            ...
            // attribute strings.
            Attr.Length = (SHORT)((Y == coordWrite.Y) ? (X - coordWrite.X + 1) : (X + 1));
            Attr.Attr = wElement & ~COMMON_LVB_SBCSDBCS;
            if (1 != Row->AttrRow.Length || memcmp(Row->AttrRow.Attrs, &Attr, sizeof(Attr)))
            {
                PATTR_PAIR NewAttrs;
                WORD NewAttrsLength;

                if (!NT_SUCCESS(MergeAttrStrings(Row->AttrRow.Attrs, Row->AttrRow.Length, &Attr, 1, &NewAttrs, &NewAttrsLength, (SHORT)(X - Attr.Length + 1), X, Row, pScreenInfo)))
                {
                    pScreenInfo->ResetTextFlags(coordWrite.X, coordWrite.Y, X, Y);
                    return STATUS_NO_MEMORY;
                }
