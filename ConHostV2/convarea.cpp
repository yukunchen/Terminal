...
RECT GetImeSuggestionWindowPos()
{
    ...
    // TODO: This could be more exact by looking up the CONVAREA but it works well enough this way.
...
NTSTATUS AllocateConversionArea(_In_ COORD dwScreenBufferSize, _Out_ PCONVERSIONAREA_INFORMATION * ConvAreaInfo)
{
    ...
    CHAR_INFO Fill;
    Fill.Attributes = g_ciConsoleInformation.CurrentScreenBuffer->GetAttributes();

    CHAR_INFO PopupFill;
    PopupFill.Attributes = g_ciConsoleInformation.CurrentScreenBuffer->GetPopupAttributes();
...
#pragma prefast(suppress:__WARNING_INCORRECT_ANNOTATION, "Precarious but this is internal-only code so we can live with it")
            ...
            if (i != 0)
            {
                ConvScreenInfo->SetAttributes(lpAtrIdx[Attr & 0x07]);
                if (Attr & 0x10)
                {
                    ConvScreenInfo->SetAttributes(ConvScreenInfo->GetAttributes() | (COMMON_LVB_GRID_SINGLEFLAG + COMMON_LVB_GRID_RVERTICAL));
                }
                else if (Attr & 0x20)
                {
                    ConvScreenInfo->SetAttributes(ConvScreenInfo->GetAttributes() | (COMMON_LVB_GRID_SINGLEFLAG + COMMON_LVB_GRID_LVERTICAL));
                }
...
NTSTATUS FillUndetermineChars(_In_ PCONVERSIONAREA_INFORMATION ConvAreaInfo)
{
    ...
    FillOutput(ConvAreaInfo->ScreenBuffer, g_ciConsoleInformation.CurrentScreenBuffer->GetAttributes(), Coord, CONSOLE_ATTRIBUTE, &CharsToWrite);
    ConsoleImePaint(ConvAreaInfo);
    return STATUS_SUCCESS;
}
...
NTSTATUS ConsoleImeWriteOutput(_In_ PCONVERSIONAREA_INFORMATION ConvAreaInfo, _In_ PCHAR_INFO Buffer, _In_ SMALL_RECT CharRegion, _In_ BOOL fUnicode)
{
    ...
    else
    {
        ...
        if (!SUCCEEDED(ULongMult(BufferSize.Y, BufferSize.X, &NumBytes)) ||
            !SUCCEEDED(ULongMult(NumBytes, 2, &NumBytes)) || !SUCCEEDED(ULongMult(NumBytes, sizeof(CHAR_INFO), &NumBytes)))
        {
                                  ...
                                  _In_reads_(StringLength) PCHAR StringA)
{
    ...
    PROW const Row = ScreenInfo->TextInfo->GetRowByOffset(TargetPoint.Y);
    ASSERT(Row != nullptr);
    ...
    CopyMemory(&Row->CharRow.Chars[TargetPoint.X], String, StringLength * sizeof(WCHAR));
    CopyMemory(&Row->CharRow.KAttrs[TargetPoint.X], StringA, StringLength * sizeof(CHAR));
    ...
    if (Row->AttrRow.Length != 1 || Row->AttrRow.Attrs->Attr != ScreenInfo->GetAttributes())
    {
        PATTR_PAIR NewAttrs;
        WORD NewAttrsLength;
        ATTR_PAIR Attrs;

        if ((ScreenInfo->GetAttributes() & (COMMON_LVB_GRID_SINGLEFLAG + COMMON_LVB_GRID_RVERTICAL)) == (COMMON_LVB_GRID_SINGLEFLAG + COMMON_LVB_GRID_RVERTICAL))
        {
            SHORT i;
            for (i = 0; i < StringLength; i++)
            {
                Attrs.Length = 1;
                if (*(StringA + i) & CHAR_ROW::ATTR_LEADING_BYTE)
                {
                    Attrs.Attr = ScreenInfo->GetAttributes() & ~(COMMON_LVB_GRID_SINGLEFLAG + COMMON_LVB_GRID_RVERTICAL);
                }
                else
                {
                    Attrs.Attr = ScreenInfo->GetAttributes() & ~COMMON_LVB_GRID_SINGLEFLAG;
                }
        ...
        else if ((ScreenInfo->GetAttributes() & (COMMON_LVB_GRID_SINGLEFLAG + COMMON_LVB_GRID_LVERTICAL)) ==
                 (COMMON_LVB_GRID_SINGLEFLAG + COMMON_LVB_GRID_LVERTICAL))
        {
            ...
            {
                Attrs.Length = 1;
                if (*(StringA + i) & CHAR_ROW::ATTR_TRAILING_BYTE)
                {
                    Attrs.Attr = ScreenInfo->GetAttributes() & ~(COMMON_LVB_GRID_SINGLEFLAG + COMMON_LVB_GRID_LVERTICAL);
                }
                else
                {
                    Attrs.Attr = ScreenInfo->GetAttributes() & ~COMMON_LVB_GRID_SINGLEFLAG;
        ...
        else
        {
            ...
            Attrs.Attr = ScreenInfo->GetAttributes();
            Status = MergeAttrStrings(Row->AttrRow.Attrs,
                                      Row->AttrRow.Length,
                                      &Attrs,
                                      1,
                                      &NewAttrs,
